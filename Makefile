...
run-all-examples:
	@echo "🚀 Запуск всех примеров Evolution LangChain..."
	@echo "📋 Проверка окружения и запуск примеров..."
	@if [ -f .env ]; then \
		echo "📄 Загружение переменных окружения из файла .env..."; \
...
run-streaming:
	@echo "🌊 Запуск примеров стриминга..."
	@if [ -f .env ]; then echo "Загружение переменных окружения из файла .env..."; export $$(grep -v '^#' .env | xargs); fi; \
